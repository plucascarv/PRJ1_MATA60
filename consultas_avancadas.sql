...
-- 2. Atividades com maior número de certificados emitidos (Corrigida)
SELECT 
    a.nm_atividade,
    COUNT(*) AS certificados_emitidos
FROM rl_participa r
JOIN tb_atividade a ON a.id_atividade = r.id_atividade
/* Tabela 3 adicionada para cumprir o requisito */
JOIN tb_participante p ON r.id_participante = p.id_participante
...
-- 3. Participantes que mais frequentaram atividades (Corrigida)
SELECT 
...
-- 4. Parceiros mais frequentes nas atividades (Corrigida)
SELECT 
    pr.nm_empresa,
...
-- 5. Média de feedback por tipo de atividade (Corrigida)
SELECT 
    a.tp_atividade,
...
-- 6. Subconsulta – participantes com mais de 3 certificados (Corrigida)
SELECT 
    p.nm_primeiro || ' ' || p.nm_ultimo AS nome_participante,
    total_certificados
FROM (
    ...
    FROM rl_participa r
    /* Tabela 3 adicionada (dentro da subconsulta) */
    JOIN tb_atividade a ON r.id_atividade = a.id_atividade
    WHERE r.is_certificado = 'S'
...
-- 9. Ranking de atividades com maior número de feedbacks (Corrigida)
SELECT 
    a.nm_atividade,
    COUNT(r.ds_feedback) AS total_feedbacks,
...
-- 10. Subconsulta – instituições com maior média de certificados (Corrigida)
SELECT 
    p.nm_instituicao,
